...
- Priority: High
- Status: Resolved in source — SDK rc.5 integrated; Space release pending
- Introduced: Unknown; observed in v0.1.46-beta.2
- Created: 2026-09-14

### Implementation Update — 2026-09-14

KodaX checks read/MCP images on receipt and prepares images once at native run admission,
including restored histories; subsequent requests reuse prepared bytes. Confirmed bad images become explanatory text in the request only;
normal images, original history/files, tool pairing, structured results and isError
remain intact. Normal MIME follows actual bytes. Temporary decoder failures are not
cached as corruption. Nested upstream error codes are retained.

Space's `saveClipboardImage` calls the new optional media validator before writing:
confirmed corruption is rejected with a re-extraction instruction, while valid WebP
still works when the old normalizer lacks its codec. Older SDKs retain the existing
fallback. Space now pins the official SDK `0.7.96-rc.5` package, including bounded,
evidence-backed text diagnosis and request repair through the existing Runtime.

SDK broad regressions and both real GLM recovery requests passed. Space clipboard
checks: 42 passed, 1 host-permission skip; Electron type check passed. Windows Node,
Bun compiled-codec sidecar and Electron ASAR decoder probes passed. Full installers
and macOS/Linux remain unverified at that initial checkpoint. **SDK rc.5 is now integrated;
customers need a Space build containing this update.** Current integration checks are
recorded in the guide below; the earlier probe results do not replace installer acceptance.

See [regression and release guide](test-guides/ISSUE_215_v0.1.46_REGRESSION_GUIDE.md).
The following investigation sections preserve evidence from before implementation.

### Original Problem

A user screenshot shows an existing Coder conversation with several follow-up prompts and
no intervening visible assistant reply. An earlier screenshot supplied afterward establishes
that the assistant had already replied and the UI recorded 16 commands before the first
visible rejection. The selected model is `glm-5.3-flash`, reasoning is automatic, and the
notice reads `Provider rejected the request.` Expected behavior is that document generation
and subsequent follow-ups complete or provide an actionable explanation of the rejection.

Reported sequence, corrected by the earlier screenshot:

1. Ask Space to use an existing Word `.doc` template for a quotation document.
2. The assistant says it will inspect the template's header, fonts, and margins, then regenerate
   the quotation. The UI groups 16 commands, including `todo_create` and `todo_update`, with
   approximately 1.4k thinking tokens. Individual command results are collapsed.
3. A `Provider rejected the request.` notice appears before the subsequent price-change prompt.
4. The user sends continue, a request to reduce the total price, and continue again. Another
   rejection appears with no intervening visible assistant answer.

The original template, history, and exact requests are unavailable, so this sequence has not
been independently reproduced. The later price-change prompt did not trigger the first
visible failure. The screenshots do not establish which command or Provider request first
failed, whether all 16 commands succeeded, or whether the collapsed notices have identical
underlying HTTP details.

### Context and Evidence

- Screenshot: Space `v0.1.46-beta.2`, community edition; macOS-style window controls.
- Failure kind: `upstream`; stage: `transport`; provider error: `upstream_client_error`;
  HTTP status: `400`.
- Request ID: `2026091410060942af7916c65f4aba`.
- User follow-up: the Provider is probably `zhipu-coding` or `zai-coding` (not confirmed),
  and the affected user's logs cannot currently be obtained. The template-based editing
  workflow is confirmed as the user's understanding of the scenario. The earlier screenshot
  confirms a command group before the rejection; full tool arguments/results remain unavailable.
- The repository pins SDK `0.7.96-rc.4`; the affected machine's loaded SDK is unverified.
- The installed SDK maps `upstream_client_error` to this generic safe message. Space's
  `runtime-host-adapter.ts` forwards the structured safe message and available diagnostics.
  The screenshot does not provide an upstream error code or original response message.
- The `1.5M` badge is cumulative Session token usage (`ContextWindowIndicator.tsx`),
  not evidence that this individual request exceeded the model context capacity.
- Existing mapping check passed: `node --test --import tsx --test-name-pattern='daemon failureDetail uses stable provider codes for recovery actions' apps/desktop/electron/test/runtime-host-adapter.test.ts`
  (1 test passed). This checks error presentation, not reproduction of the reported rejection.

### Initial Investigation Questions (before customer analysis)

1. Confirm whether the configured Provider is `zhipu-coding` or `zai-coding` and whether
   its endpoint has been overridden.
2. Obtain the redacted upstream `error.message` / `error.code` associated with the Request ID,
   plus the affected Runtime/SDK version. Space Settings > Diagnostics can export a redacted
   main-process bundle, but that bundle may not contain the upstream response body; use the
   Provider or intermediary's request logs if needed. Never include API keys.
3. Check whether a fresh conversation with the same Provider/model succeeds. If it does,
   inspect the failing history/request for message, tool-result, reasoning, and context errors;
   if it also fails, inspect endpoint/model access and request parameters.
4. Reproduction must include the template inspection/tool phase and a subsequent model request,
   then a new follow-up after failure. A plain first-message success alone cannot exercise this
   sequence. Compare the requests before and after the command group to distinguish invalid
   message/tool/reasoning history, excessive or unsupported tool content, and upstream rejection
   unrelated to history. These are investigation candidates, not confirmed causes.

### SDK Investigation — 2026-09-14

The user suspected the shared Anthropic ordering change in SDK commit `b25c5142`.
Controlled live requests to both `zhipu-coding` and `zai-coding` accepted current and
legacy assistant ordering, 16 tool calls, split/interrupted tool history, and ordinary
image results. Real SDK streaming tool/reply/follow-up sequences also succeeded, including
automatic reasoning with actual thinking blocks.

A separate chain was reproduced on both Providers: `toolRead()` accepts a `.png` file
containing non-image bytes, the following Provider call fails with HTTP 400 / `1210`
(image parsing error), and appending continue fails again. Keeping history and ordering
unchanged while replacing that file's bytes with valid PNG data restores success.
The SDK also drops the nested upstream `1210` code from error metadata, leaving the
generic Runtime message seen in this report. The defective image path predates the
ordering change. At this stage, the screenshots alone did not establish that the customer
read a bad image.

This confirmed synthetic SDK defect is tracked as KodaX Issue 335. See the
[SDK investigation and opt-in real-provider reproducer](../../KodaX/docs/investigations/GLM_CODING_400_2026-09-14.md).

### Customer Analysis Update — 2026-09-14

The user relayed a customer analysis of session
`20260914_094401_es887b8d024364.jsonl`. The initial quotation docx completed at 09:49.
After the header-template follow-up, the agent extracted images from the legacy `.doc`
and read `img/img_6433.jpg` at 09:56:12. Subsequent requests failed with kimi-code
`failed to decode image: invalid or unsupported image format`, then three zhipu-coding
`1210 / 图片输入格式/解析错误` rejections. Continue and price-change prompts did not recover.

The analysis reports JFIF/EXIF headers but no SOF frame data in `img_6433.jpg`, and PIL
could not identify it. The other two images, `img_11609.jpg` and `img_6765.jpg`, were
valid 160×46 Baseline JPEGs. These are supplied findings; the original JSONL/JPEG files
have not been independently inspected here.

This evidence strongly corroborates SDK Issue 335's independently reproduced cause:
invalid image content is accepted by `read`, retained as a historical reference, and
resubmitted on later requests. It supports moving this incident to `ready`. Current
evidence does not support rolling back the Anthropic ordering change.

### Next Steps

1. Fix SDK image admission with decode validation; extension, MIME, and magic-byte checks
   alone cannot catch this customer's reported JPEG. Add missing-SOF and truncated-image
   regressions with valid JPEG controls.
2. Handle invalid images already present in history when preparing requests, with explicit
   unavailable-image information while preserving other content and the original session.
3. Surface safe image-specific upstream diagnostics so Space can explain the failure and
   recovery action; verify continued use of the affected history after SDK integration.
4. Cover pasted-image admission too: `ipc/clipboard.ts` currently writes raw bytes after
   normalization errors when within its size budget. Distinguish confirmed corrupt images
   from unsupported codecs/backend failures so valid formats (especially WebP) remain usable.
   The same SDK scope includes returned MCP image/resource content; preserve valid images,
   text, structuredContent and original tool error status. This does not disable MCP tools.

The [pi/codex comparison and expanded recovery design](../../KodaX/docs/investigations/IMAGE_COMPATIBILITY_RECOVERY_2026-09-14.md)
documents 27 offline checks, real-provider request-copy recovery, codec compatibility
controls and required packaged-runtime checks. Preserve overall feature availability;
do not treat every local decoder failure as proof that an image is corrupt.

No production runtime behavior has been changed and no customer fix is claimed.
...
- FEATURE_274 hardening layers identity evidence **above** this guard: a
  ...
  persisted nothing. The raced user-only read therefore holds as _pending_
