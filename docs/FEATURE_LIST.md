...
| `v0.1.46-alpha.11` | KodaX 0.7.96-rc.2 source alignment (permission authority v6, capability-gated Session Stop) + FEATURE_275 transcript ownership landing | Maintenance / SDK alignment + transcript rewrite | Exact rc.2 Registry bytes are pinned and startup/daemon/packaging gates now require `runtimeAutoModeGuardrail:6`; embedded runs pass their bound permission mode to SDK text authority so Runtime and direct-SDK text writes agree. Owners advertising `sessionCancellation:1` provide durable Session Stop frontiers; rc.2 rejects stale first requests atomically while preserving accepted-request replay. Result-unknown stops retain the original Session/Run/requestId. The rc.2 daemon still supports only the bound-Run Stop fallback, whose terminal retry does not target successor Runs. Managed extension and explicit `!command` Runs require `toolInvocation:1`, currently absent from the daemon; `/repair-identity` applies only user-verified audited mappings. FEATURE_275 tickets 1-6 land the identity-anchored transcript model: canonical pages plus a transient live tail with explicit input/output ownership, so settling one input can no longer delete sibling inputs in the same Run, and retirement receipts reject journal replay or snapshot sidecars without treating a larger cursor as coverage proof. Packaged boot keeps restart-stable Runtime credentials in an isolated keyring namespace; `identity_repair_invalid` and structured `local_execution` failures preserve readable history without provider-setup suggestions. |
