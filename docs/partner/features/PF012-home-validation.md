...
## 2026-09-20 场景切换提示词修复（分支内，未发布）

- 原因：首页任务卡使用通用的光标插入事件，连续选择“调研一个主题”“分析一份数据”时会把提示词叠加。
- 修复：专家绑定成功后，复用输入框预填事件替换整份当前草稿。重复选择同一个场景也只保留一份提示词，不自动创建会话或发送消息。
- 回归边界：`PartnerStarterTasks.test.ts` 运行真实场景卡、专家 Provider 和 `BottomBar`，只替代外部 IPC 与 Vite 资源环境。无头浏览器已复现研究提示词与分析提示词拼接的失败；修复后连续切换、重复点击、六种场景、通用方法插入、绑定失败和跨项目异步返回均通过。
- 最终验证：在自己的 `feature/partner-maintenance-20260917` 分支吸收 `origin/main@52f94c1d`（Space `0.1.46-rc.1` / SDK `0.7.96-rc.8`）后，`typecheck`、`lint`、`build:smoke` 和完整 `npm test` 均通过；测试共 4203 项通过、5 项跳过、0 项失败，本次场景切换回归实际执行通过。未修改 `main`，未生成或发布安装包。
- 人工复验：在 Partner 首页依次选择研究→数据→研究→数据，再重复点数据；每次输入框应只有对应场景的一份提示词，专家标签与场景一致，仍需用户主动发送。切换场景会替换当前草稿，包括用户对草稿的编辑。
- 未启动桌面应用或使用真实账号。以下 2026-09-08 记录保留当时事实，其中旧的“插入并保留草稿”语义由本修订替代。
