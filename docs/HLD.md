...
>
> **2026-09-17 当前源码候选**：Space package 为 `0.1.46-rc.1`，root/Desktop/lockfile 精确锁定 KodaX
> `0.7.96-rc.8`。rc.8 让 Space 保留用户推理意图，SDK 负责档位协商与拒绝缓存，daemon 与 embedded 双路径转发 `reasoning_resolved` 观测（verified:false），选择器单独显示上次发送档位与回退原因。SDK 包启动门要求 `effectiveConfig:1`、`sandboxRuntime:11`、`runtimeAutoModeGuardrail:6` 与 `sharedSessionSettings:2`；`providerCredentialBroker:2` 由 daemon 准入 requirements 和连接后 Runtime capability
