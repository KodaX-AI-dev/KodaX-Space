...
### 2026-09-22 产物预览高度修正（分支内，未发布）

右侧详情标签页已有完整高度，但 `ArtifactPanel` 未继承标签页高度，导致 Markdown/HTML 的 iframe 退回 150px 默认高度。产物面板显式占满父容器，既有工具栏与预览继续按 flex 分配空间，长内容在预览内部滚动。

`ArtifactPanel.layout.test.ts` 用真实右侧栏、产物读取/版本切换、渲染组件和生产样式，在无头 Chromium 中复现“958px 标签页只显示 150px 预览”的失败。修复后验证 Markdown 长报告滚动、短版本、HTML、窗口变高/变矮、标签切换与产物汇总入口，预览均填满工具栏下方的可用空间。IPC 使用隔离测试数据，未启动桌面应用或读取用户报告。

验证通过：`typecheck`、`lint`、`build:smoke`、完整 `npm test`（4204 项通过、5 项跳过、0 项失败；新增布局回归实际运行通过）。未进行桌面启动或安装包发布验收。

人工复验：从 Partner 对话产物卡打开一份长报告，确认内容延伸到右侧面板底部；缩放窗口、切换版本及往返切换产物标签后应保持完整高度，报告可独立滚动到末尾。此次修正只交付到 `feature/partner-maintenance-20260917`，不代表安装包已更新。
...
2026-09-20 修订：首页场景卡在专家绑定成功且仍属于当前上下文时，使用既有输入框预填操作替换当前草稿。连续切换或重复选择场景只保留当前场景的提示词，不按光标追加，也不自动发送。绑定失败、项目/会话上下文变化或组件卸载时不替换草稿；其他方法和连接器的快捷插入行为保持不变。
