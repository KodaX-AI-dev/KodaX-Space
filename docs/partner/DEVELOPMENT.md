...
2026-09-14 本地准备完成后，用户已明确授权将 `integration/partner-bundled-release` 上传到组织仓库。组织 `origin` 的 push URL 可为此次上传恢复；当前授权仅包含该集成分支，不包含创建 PR、合并主线、推送标签或发布 Release。
...
当前已完成本地准备并取得本次组织分支上传授权。按以下步骤区分代码交付和产品发布：
