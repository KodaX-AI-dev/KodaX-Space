...
| `v0.1.46-alpha.11` | KodaX 0.7.96-rc.3 source alignment (permission authority v6, daemon Session Stop and explicit tools) + FEATURE_275 transcript ownership landing | Maintenance / SDK alignment + transcript rewrite | Exact rc.3 Registry bytes are pinned and startup/daemon/packaging gates require `runtimeAutoModeGuardrail:6`; embedded runs pass their bound permission mode to SDK text authority so Runtime and direct-SDK text writes agree. Owners advertising `sessionCancellation:1` provide durable Session Stop frontiers; rc.2 rejects stale first requests atomically while preserving accepted-request replay. Result-unknown stops retain the original Session/Run/requestId. rc.3 daemon public-API tests verify current/queued cancellation, replay, stale rejection, successor survival and explicit write/bash. Space requires both capabilities, removes the rc.2 Run-only fallback, and recognizes nested daemon stale errors. Old-owner upgrade stays SDK-owned. rc.3 packaged Runtime, boot, exit and live control paths pass; blue-PNG color recognition remains inconsistent. Managed extension and explicit `!command` Runs use `toolInvocation:1`; `/repair-identity` applies only user-verified audited mappings. FEATURE_275 tickets 1-6 land the identity-anchored transcript model: canonical pages plus a transient live tail with explicit input/output ownership, so settling one input can no longer delete sibling inputs in the same Run, and retirement receipts reject journal replay or snapshot sidecars without treating a larger cursor as coverage proof. Packaged boot keeps restart-stable Runtime credentials in an isolated keyring namespace; `identity_repair_invalid` and structured `local_execution` failures preserve readable history without provider-setup suggestions. |
