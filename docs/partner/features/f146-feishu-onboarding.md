...
### 2026-09-22 会话连接校验恢复修正（分支内，未发布）

用户遇到“已连接”同时提示“账号已变化”，“去试试”不可用。排查发现，宿主曾把组件不可用、版本不兼容、验证失败和身份变化统一报为账号变化；首次选择失败时尚未保存任何绑定，后续刷新却直接返回而保留旧错误，弹窗也缺少重试入口。

修正后，宿主保留经过脱敏的验证失败原因，只有已验证身份与原账号不符才报账号变化；连接弹窗提供“重新检查会话连接”。已有会话重新读取原绑定，已有草稿绑定重新验证；空草稿清除失败状态但不增加任何权限，下次“去试试”仍须经过宿主完整校验。保留原账号、revision、文档范围和输入草稿，不自动断连、重授权、创建会话或发送消息。

自动回归覆盖首次失败后恢复、已有资料范围的重试保留、真实弹窗重试、组件/版本/验证/身份错误区分。2026-09-22 本机只读核验：托管 CLI 1.0.92 返回已验证身份，应用 ID 与用户 ID 均匹配 Space 当前账号；此证据不代表执行过远端文档读写，也不能追溯原先那次失败的具体外部原因。

验证结果：`npm run typecheck`、`npm run lint`、`npm run build:smoke` 通过；不启动浏览器的 service 与 binding 定向测试 47/47 通过。新增的两个弹窗恢复用例此前已在真实组件、模拟宿主的浏览器测试中通过。完整 `npm test` 在执行过程中因用户要求停止反复启动浏览器而主动中止，不能记为完整通过；未重启桌面前端或补做真实桌面交互，剩余界面回归留给 CI 或用户后续安排的验收。

后续补验与澄清（同日）：用户允许为修复问题打开浏览器或前端，要求避免重复启动。已修正规范，启动一次开发版并复用同一窗口。真实桌面显示当前会话的飞书账号入口；只读核对确认会话已保存该账号绑定、revision 一致，CLI 再次验证身份与原账号匹配。当前文档任务随后被模型服务商的 API Key 鉴权失败阻断，不能把此结果记作飞书文档创建成功；未代用户修改模型密钥、重新授权飞书或操作远端文档。界面重试失败场景的证据仍为前述组件测试，完整回归未补跑。
