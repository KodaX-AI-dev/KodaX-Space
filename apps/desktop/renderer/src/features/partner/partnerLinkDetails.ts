...
import { partnerDetailTargetForNativeDocument } from './partnerNativeDocumentAutoOpen.js';
  ...
  useEffect(() => {
    const open = (detail: PartnerLinkDetailRequest): void => {
      ...
      if (
        ...
        return;
      const isHttp = /^https?:\/\//i.test(detail.href);
      ...
      const task = records.documentTasks.find(
        (item) =>
          item.status === 'succeeded' &&
          item.projectRoot === projectRoot &&
          item.sessionId === sessionId &&
          item.canonicalUrl === detail.href,
      );
      if (task) {
        onOpen(partnerDetailTargetForNativeDocument({ kind: 'native-document', task }));
        return;
      }
      ...
      if (isHttp) {
        const target = partnerDetailTargetForWebPage(detail.href);
        if (target) onOpen(target);
        else pushToast(translateMessage('webPreview.remoteFailed'), 'info');
        return;
      }
