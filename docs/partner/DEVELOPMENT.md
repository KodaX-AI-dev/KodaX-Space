...
2026-09-22 本地验收约束：用户要求不要反复打开浏览器。日常后台检查不自动启动浏览器、浏览器自动化测试或桌面前端，包括可能造成启动干扰的无头系统 Chrome。只有用户后续明确要求时再启动；完整界面回归留给 CI 或用户安排的验收。默认 `npm test` 会启动浏览器组件测试，不能将它当作无浏览器检查；受到此约束时，运行不启动浏览器的定向测试，并如实记录完整回归未完成。
