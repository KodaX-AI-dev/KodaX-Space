...
>
> **2026-09-14 当前源码候选**：Space package 为 `0.1.46-beta.4`，root/Desktop/lockfile 精确锁定 KodaX
> `0.7.96-rc.5`。SDK 包启动门要求 `effectiveConfig:1`、`sandboxRuntime:11`、`runtimeAutoModeGuardrail:6` 与 `sharedSessionSettings:2`；`providerCredentialBroker:2` 由 daemon 准入 requirements 和连接后 Runtime capability
