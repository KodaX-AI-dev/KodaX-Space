...
>
> **2026-09-23 当前源码候选**：Space package 为 `0.1.46-rc.4`，root/Desktop/lockfile 精确锁定 KodaX
> `0.7.96-rc.11`。rc.8 让 Space 保留用户推理意图，SDK 负责档位协商与拒绝缓存，daemon 与 embedded 双路径转发 `reasoning_resolved` 观测（verified:false），选择器单独显示上次发送档位与回退原因。rc.2 又接入 rc.9 的 Issue 216 修复面：Windows 凭据 vault（safeStorage/DPAPI）、Runtime 身份保留、Auto Shell 启动探针、updater 生产依赖与结构化脱敏诊断（含 SDK 日志桥）。SDK 包启动门要求 `effectiveConfig:1`、`sandboxRuntime:11`、`runtimeAutoModeGuardrail:6` 与 `sharedSessionSettings:2`；`providerCredentialBroker:2` 由 daemon 准入 requirements 和连接后 Runtime capability
