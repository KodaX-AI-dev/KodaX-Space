...
- 移除窗口顶部重复的全局左侧栏收展按钮，保留 Partner 标题旁与 Coder 页面内的收展按钮；保留任务上下文和右侧详情栏的独立控制。
- Partner 在线文档成果、聊天网页链接与资料/多维表格的网页入口统一使用 Space 共享 `ProjectWebPreview`，在右侧显示网页并支持原位重试；重复点击同一文档成果/聊天链接复用标签。不再自动跳转系统浏览器，不恢复 `PartnerBrowserPanel`。已保存资料仍优先展示会话快照，网站登录与嵌入限制以实际提供方页面为准。
- 连接器会话校验失败后可在连接弹窗“重新检查会话连接”，恢复后继续使用原账号与资料范围；首次选择失败不再留下无法清除的错误。飞书组件、验证失败与账号变化分别提示，不因临时失败自动断开或重新授权。
