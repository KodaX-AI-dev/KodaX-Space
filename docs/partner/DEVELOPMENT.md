...
2026-09-22 本地验收约束（用户澄清）：允许为修复问题打开浏览器或桌面前端，但不要反复打开。优先复用现有浏览器会话、标签页和桌面实例；确需启动时只启动一次，并在同一窗口完成验证。此前“不允许打开浏览器或前端”的表述是误解，已撤销。默认 `npm test` 会反复启动浏览器组件测试，本机优先运行必要的定向检查，广泛界面回归可交给 CI；如实记录未完成项，重启前检查是否有授权或写入进行中。
