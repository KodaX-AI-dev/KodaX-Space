...
>
> 当前源码候选为 Space `0.1.46-rc.2`，精确锁定 KodaX `0.7.96-rc.9`，并要求 `sessionCancellation:1`（durable frontier）、`toolInvocation:1`、`sandboxRuntime:11`、`runtimeAutoModeGuardrail:6`、`sharedSessionSettings:2`、`providerCredentialBroker:2` 与 `effectiveConfig:1`。
...
> v0.1.46-beta.5 要求 owner 公布 `toolInvocation:1`，受管扩展命令与 `!command` 可在会话空闲时通过正常 Run 执行，沿用权限、事件、历史和取消语义。rc.3 发布包 daemon 的显式 write/bash 测试已通过；正常模型调用工具不受影响。纯配置型扩展命令请在 KodaX CLI owner 中执行。
...
beta.5 的安装包同时携带 SDK 内置的 Windows WFP 探针端口分配修复。Space 直接复制该运行库及其依赖，构建时跳过安装脚本也不会丢失修复；Windows native protocol/setup generation 和公开能力版本均不变。
...
- 确认损坏的图片会提示重新提取或替换。旧会话中的坏图在模型请求里改为说明文字，其他正常图片和文字保留，原会话和文件不被改写；替换文件后重新读取即可重新校验。有效 WebP 不会仅因归一化器缺少对应解码支持而被拒绝。
...
`0.1.46-rc.2` 候选新增随 Space 分发的 Partner 插件库。安装包含本次改动的版本后，首次启动会安装并启用内置库，无需单独下载 `.space-extension`。正式发布状态见 [版本验收记录](releases/v0.1.46-beta.3-release-readiness.md)；稳定版 `v0.1.45` 不包含这一增量。

1. 切换到 **Partner**，从首页任务卡直接选择专家，或通过 **Plugins（插件）** 打开专家库选择。
2. 选择专家，说明目标并添加资料。专家的方法会用于当前 Partner 会话，继续使用 Space 中配置的模型。
3. 专家选择随会话保存；恢复会话后继续使用，也可主动切换或移除。专家被停用或方法不可用时，按界面提示处理，不把目录可见理解为任务已完成。

首页任务卡在专家选择成功后，用该场景提示词替换输入框的当前草稿（包括已编辑内容）；切换或重复点击不会叠加提示词，也不会自动发送。选择失败时保留原草稿。

### 12.2 连接外部资料

若连接弹窗显示已连接，但会话校验报错且“去试试”不可用，可点击“重新检查会话连接”。恢复后继续使用原账号与已保存范围；若仍提示账号身份变化或授权失效，再按提示重新连接。重试不会自动发送消息或操作远端文档。
...
Coder 的普通网页链接由系统浏览器打开。Partner 的在线文档成果和 HTTPS 网页链接在右侧共享网页预览中打开；资料快照和多维表格任务详情可点击“在右侧预览打开”。同一文档成果与聊天链接重复点击会复用标签，加载失败可在预览区重新加载。网站可能需要单独登录或限制嵌入，连接器已连接不代表网页已登录。已保存资料优先显示本会话快照，本地文件继续使用现有文件查看器；恢复历史不会自动打开网页。
...
可以在插件管理中停用或卸载 Partner 库。Space 会记住该选择，重启不会重新启用或自动装回。已有同 ID 的手动安装包及其启停状态会保留，首次随包注册不会覆盖它。此次交付没有实现随 Space 自动替换已有插件版本的策略。
