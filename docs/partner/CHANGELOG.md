...
- Partner 右侧产物预览占满标签页的可用高度，修复 Markdown/HTML 报告仅在顶部约 150px 区域显示的问题；调整窗口、切换标签或版本后保持完整预览区域。
- Partner 首页切换或重复选择任务场景时，用当前场景提示词替换输入框草稿，避免旧提示词反复叠加；专家绑定失败或上下文已改变时保留草稿，不自动发送。
